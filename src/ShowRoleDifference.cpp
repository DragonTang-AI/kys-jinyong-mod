...
void ShowRoleDifference::draw()
{
    ...
    showOneDifference(role1_->HitRate, "命中 {:9}     -> {:9}", 20, color, x, y);
    showOneDifference(role1_->Dodge, "閃避 {:9}     -> {:9}", 20, color, x, y);
    showOneDifference(role1_->CriticalRate, "暴擊 {:9}     -> {:9}", 20, color, x, y);
