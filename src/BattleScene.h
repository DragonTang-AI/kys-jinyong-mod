...
// 自动战斗标志，由 UIGameAssist.cpp 中的 g_battle_auto 提供
extern int g_battle_auto;
...
protected:
    ...
    int effect_frame_ = 0;
    int dead_alpha_ = 255;
    static int animation_delay_;  // 可被 g_battle_speed 覆盖，默认 2
    static int getAnimationDelay() { return animation_delay_; }
    ...
    bool show_enemy_info_ = false;   // Tab 查看敌人属性面板
    void drawEnemyInfoPanel();       // 绘制敌人属性面板
...
public:
    ...
    static void setAnimationDelay(int d) { animation_delay_ = d; }  // 供 UIGameAssist 设置
    ...
    virtual int calComboCount(Role* r);                                                                  //计算连击次数
    virtual int calMagiclHurtAllEnemies(Role* r, Magic* m, bool simulation = false, float comboDamageRatio = 1.0f);    //计算全部人物的伤害
