...
UIConfig::UIConfig()
{
    ...
    addOption("遇怪開關", "mainmap_encounter", "enable", { "關閉", "開啟" });
...
void UIConfig::loadConfig()
{
    ...
    items_[6].value = GameUtil::getInstance()->getInt("mainmap_encounter", "enable", 1) ? 1 : 0;
}
...
void UIConfig::saveConfig()
{
    ...
    GameUtil::getInstance()->setKey("mainmap_encounter", "enable", std::to_string(items_[6].value));
