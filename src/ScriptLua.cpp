#include "ScriptLua.h"
#include "ScriptCifa.h"  // ScriptExitException
#include "Event.h"
...
int ScriptLua::runScriptString(const std::string& content)
{
    ...
    if (r == 0)
    {
        ...
        {
            return 0;  // exit() / forceexit() 正常终止，不算错误
        }
...
#define REGISTER_INSTRUCT(function) REGISTER_INSTRUCT_ALIAS(#function, function)
    ...
    // exit - 立即终止脚本执行（与 Cifa 保持一致）
    auto exit_func = [](lua_State* L) -> int
    {
        throw ScriptExitException{};
        return 0;
    };
    lua_register(lua_state_, "exit", exit_func);
    lua_register(lua_state_, "forceexit", exit_func);
...
#define FOR_EXIT_TO_TITLE \
    ...
    auto cloneRole = [](lua_State* L) -> int
    {
        int dst = int(luaL_checkinteger(L, 1));
        int src = int(luaL_checkinteger(L, 2));
        auto* gu = GameUtil::getInstance();
        std::string name = gu->getString("shadow_dongfang", "shadow_name", "东方不败（影）");
        std::string nick = gu->getString("shadow_dongfang", "shadow_nick", "葵花照影");
        Save::getInstance()->cloneRoleFromTo(dst, src, name, nick);
        return 0;
    };
    lua_register(lua_state_, "clonerole", cloneRole);
