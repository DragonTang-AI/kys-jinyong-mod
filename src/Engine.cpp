...
int Engine::init(void* handle /*= nullptr*/, int handle_type /*= 0*/, int maximized, const std::string& str)
{
...
#if defined(__ANDROID__) || defined(KYS_MOBILE_EXTRACT_ASSETS)
    SDL_SetHint(SDL_HINT_ORIENTATIONS, "LandscapeLeft LandscapeRight");
#endif

#if defined(__APPLE__) && defined(TARGET_OS_IPHONE) && TARGET_OS_IPHONE
    /* 默认为 ambient：静音开关打开时整条混音静默；playback 才把游戏音效当「媒体」播出来 */
    SDL_SetHint(SDL_HINT_AUDIO_CATEGORY, "playback");
    /* 与全面屏全屏窗口一致的首页指示器策略（系统默认也会对全屏如此） */
...
#if defined(KYS_MOBILE_EXTRACT_ASSETS)
        // iOS/Android：使用物理像素后备缓冲，避免按逻辑分辨率缩放后整屏发糊
        props.set(SDL_PROP_WINDOW_CREATE_HIGH_PIXEL_DENSITY_BOOLEAN, true);
#endif
        return SDL_CreateWindowWithProperties(props.id());
    };
...
#else
#if defined(KYS_MOBILE_EXTRACT_ASSETS)
    /* iOS：Scene/全屏尚未稳定时 GetDisplayBounds 常得到 480×320 等错误值，会污染 setWindowSize 的上限并加重卡顿 */
    min_x_ = 0;
    min_y_ = 0;
    max_x_ = 65535;
    max_y_ = 65535;
#else
    // SDL3：第一个参数是 SDL_DisplayID，不能传 0（0 表示无效），否则 bounds 未定义，日志会出现巨大乱数。
    Rect r{};
    ...
    if (display_id != 0 && SDL_GetDisplayBounds(display_id, &r))
    {
        min_x_ = r.x;
        min_y_ = r.y;
        max_x_ = r.w + r.x;
...
#if defined(KYS_MOBILE_EXTRACT_ASSETS)
    int log_w = 0, log_h = 0;
    if (SDL_GetWindowSizeInPixels(window_, &log_w, &log_h))
    {
        std::print("mobile window pixels (pre-fullscreen): {} x {}\n", log_w, log_h);
    }
#endif
    ...
    int pw = 0, ph = 0;
    if (SDL_GetWindowSizeInPixels(window_, &pw, &ph))
    {
        std::print("mobile window pixels (fullscreen): {} x {}\n", pw, ph);
    }
...
void Engine::setPresentPosition(Texture* tex)
{
    ...
    int w_dst = 0, h_dst = 0;
    int w_src = 0, h_src = 0;
    if (renderer_ && SDL_GetCurrentRenderOutputSize(renderer_, &w_dst, &h_dst))
    {
        // 与最终贴图到交换链/后备缓冲的坐标系一致（高 DPI 下优于 SDL_GetWindowSize）
    }
    else
    {
...
{
    const SDL_WindowFlags state = SDL_GetWindowFlags(window_);
    full_screen_ = (state & SDL_WINDOW_FULLSCREEN) != 0;
    return full_screen_;
}
...
void Engine::toggleFullscreen()
{
    ...
    // 全屏切换后需要重新计算渲染坐标，否则 rect_ 仍保持窗口模式 letterbox 值
    setPresentPosition(tex_);
    renderClear();
}
...
void Engine::getMouseState(int& x, int& y)
{
    ...
    Renderer* r = getInstance()->getRenderer();
    if (r && SDL_RenderCoordinatesFromWindow(r, xf, yf, &xf, &yf))
    {
        // 与 setPresentPosition / windowToUISpace 使用同一套渲染坐标
    }
    x = static_cast<int>(std::lround(xf));
    y = static_cast<int>(std::lround(yf));
}

bool Engine::fingerNormalizedToUISpace(float nx, float ny, int& ux, int& uy, bool clamp) const
{
    int win_w = 0;
    int win_h = 0;
    ...
    if (renderer_)
    {
        float rx = wx;
        float ry = wy;
        if (SDL_RenderCoordinatesFromWindow(renderer_, wx, wy, &rx, &ry))
        {
            wx = rx;
            wy = ry;
        }
    }
    return windowToUISpace(static_cast<int>(std::lround(wx)), static_cast<int>(std::lround(wy)), ux, uy, clamp);
}
...
void Engine::uiToWindowSpace(int ux, int uy, int& wx, int& wy) const
{
    ...
    ux = std::clamp(ux, 0, ui_w_ - 1);
    uy = std::clamp(uy, 0, ui_h_ - 1);
    const float rx = static_cast<float>(px + ux * pw / ui_w_);
    const float ry = static_cast<float>(py + uy * ph / ui_h_);
    float win_x = rx;
    float win_y = ry;
    if (renderer_ && SDL_RenderCoordinatesToWindow(renderer_, rx, ry, &win_x, &win_y))
    {
        wx = static_cast<int>(std::lround(win_x));
        wy = static_cast<int>(std::lround(win_y));
        return;
    }
    wx = static_cast<int>(std::lround(rx));
    wy = static_cast<int>(std::lround(ry));
}
...
std::string mobile_storage_root()
{
#if defined(__ANDROID__)
    return std::string(SDL_GetAndroidInternalStoragePath());
#else
    char* p = SDL_GetPrefPath("kys-cpp", "kys");
    std::string r = p ? p : "";
    SDL_free(p);
    return r;
#endif
...
{
    std::string internal = mobile_storage_root();
    if (!internal.empty() && internal.back() != '/')
    {
        internal += '/';
    }
...
#if defined(__ANDROID__)
    io = SDL_IOFromFile("game.zip", "rb");
#else
    std::string zip_path;
    if (const char* base = SDL_GetBasePath())
    {
        zip_path = std::string(base) + "game.zip";
    }
    if (!zip_path.empty())
    {
        io = SDL_IOFromFile(zip_path.c_str(), "rb");
    }
#endif
    if (!io)
    {
        SDL_Log("kys-cpp: cannot open bundled game.zip: %s", SDL_GetError());
        return;
    }
