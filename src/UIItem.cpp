    ...
    if (select_user_)
    {
        ...
        else if (current_item_->ItemType == 1 || current_item_->ItemType == 2)
        {
            ...
            if (role && role->canUseItem(current_item_))
            {
                if (current_item_->ItemType == 1)
                {
                    // 装备预览：模拟装备后属性变化，用户确认才真正装备
                    Role r_before = *role;
                    // 对副本模拟装备效果：仅添加装备属性差值
                    Role r_preview = *role;
                    // 先移除当前同槽位装备的属性
                    auto old_equip_id = (current_item_->EquipType == 0) ? role->Equip0 : role->Equip1;
                    auto old_equip = Save::getInstance()->getItem(old_equip_id);
                    if (old_equip)
                    {
                        r_preview.PhysicalPower -= old_equip->AddPhysicalPower;
                        r_preview.HP -= old_equip->AddHP; r_preview.MaxHP -= old_equip->AddMaxHP;
                        r_preview.MP -= old_equip->AddMP; r_preview.MaxMP -= old_equip->AddMaxMP;
                        r_preview.Poison -= old_equip->AddPoison;
                        r_preview.Medicine -= old_equip->AddMedicine;
                        r_preview.Detoxification -= old_equip->AddDetoxification;
                        r_preview.UsePoison -= old_equip->AddUsePoison;
                        r_preview.Attack -= old_equip->AddAttack;
                        r_preview.Defence -= old_equip->AddDefence;
                        r_preview.Speed -= old_equip->AddSpeed;
                        r_preview.Fist -= old_equip->AddFist;
                        r_preview.Sword -= old_equip->AddSword;
                        r_preview.Knife -= old_equip->AddKnife;
                        r_preview.Unusual -= old_equip->AddUnusual;
                        r_preview.HiddenWeapon -= old_equip->AddHiddenWeapon;
                        r_preview.Knowledge -= old_equip->AddKnowledge;
                        r_preview.Morality -= old_equip->AddMorality;
                        r_preview.AntiPoison -= old_equip->AddAntiPoison;
                        r_preview.AttackWithPoison -= old_equip->AddAttackWithPoison;
                        if (old_equip->AddAttackTwice) { r_preview.AttackTwice = 0; }
                    }
                    // 再添加新装备属性
                    r_preview.PhysicalPower += current_item_->AddPhysicalPower;
                    r_preview.HP += current_item_->AddHP; r_preview.MaxHP += current_item_->AddMaxHP;
                    r_preview.MP += current_item_->AddMP; r_preview.MaxMP += current_item_->AddMaxMP;
                    r_preview.Poison += current_item_->AddPoison;
                    r_preview.Medicine += current_item_->AddMedicine;
                    r_preview.Detoxification += current_item_->AddDetoxification;
                    r_preview.UsePoison += current_item_->AddUsePoison;
                    r_preview.Attack += current_item_->AddAttack;
                    r_preview.Defence += current_item_->AddDefence;
                    r_preview.Speed += current_item_->AddSpeed;
                    r_preview.Fist += current_item_->AddFist;
                    r_preview.Sword += current_item_->AddSword;
                    r_preview.Knife += current_item_->AddKnife;
                    r_preview.Unusual += current_item_->AddUnusual;
                    r_preview.HiddenWeapon += current_item_->AddHiddenWeapon;
                    r_preview.Knowledge += current_item_->AddKnowledge;
                    r_preview.Morality += current_item_->AddMorality;
                    r_preview.AntiPoison += current_item_->AddAntiPoison;
                    r_preview.AttackWithPoison += current_item_->AddAttackWithPoison;
                    if (current_item_->AddAttackTwice) { r_preview.AttackTwice = 1; }
                    if (current_item_->ChangeMPType == 2) { r_preview.MPType = 2; }

                    // 显示预览对比
                    auto df = std::make_shared<ShowRoleDifference>(&r_before, &r_preview);
                    df->setText(std::format("{}裝備{} [預覽]", role->Name, current_item_->Name));
                    int result = df->run();
                ...
                else
                {
                    ...
                    Role r_before = *role;
                    role->equip(current_item_);
                    auto df = std::make_shared<ShowRoleDifference>(&r_before, role);
                    df->setText(std::format("{}修煉{}", role->Name, current_item_->Name));
                    df->run();
                }
            }
        }
