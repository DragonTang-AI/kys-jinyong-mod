    ...
    for (int sum = -view_sum_region_; sum <= view_sum_region_ + 20; sum++)
    {
        for (int i = -view_width_region_; i <= view_width_region_; i++)
        {
            ...
            if (!isOutLine(ix, iy))
            {
                ...
                // 初始庭院（子地图70）：东方不败「影子」叠加绘制（半透明），不占用地编事件格
                if (submap_id_ == 70)
                {
                    auto* gu = GameUtil::getInstance();
                    if (gu->getInt("shadow_dongfang", "enable", 1) != 0)
                    {
                        const int shadow_idx = gu->getInt("shadow_dongfang", "clone_role_index", 316);
                        if (!Event::getInstance()->inTeam(shadow_idx))
                        {
                            int shx = gu->getInt("shadow_dongfang", "tile_x", 27);
                            int shy = gu->getInt("shadow_dongfang", "tile_y", 35);
                            if (ix == shx && iy == shy)
                            {
                                int pic_doubled = gu->getInt("shadow_dongfang", "sprite_pic_doubled", 6078);
                                int pic = pic_doubled / 2;
                                if (pic > 0)
                                {
                                    TextureManager::getInstance()->renderTexture("smap", pic, p.x, p.y - h, { { 255, 255, 255, 128 }, 128, 1, 1, 0, 0, color_v, brighness_v });
                                }
                            }
                        }
                    }
                ...
                // 初始庭院（子地图70）：令狐冲的剑魂
                drawSwordSoulNPC(ix, iy, p.x, p.y - h, h, color_v, brighness_v);
                //装饰
...
void SubScene::drawPositionInfo()
{
    std::string name = submap_info_ ? submap_info_->Name : "未知";
    auto text = std::format("{} ({}, {})", name, man_x_, man_y_);
    ...
    auto money_text = std::format("银两: {}", money);
    Font::getInstance()->draw(money_text, 20, Engine::getInstance()->getUIWidth() - 320, 45, { 255, 215, 0, 255 });
...
bool SubScene::checkEvent(int x, int y, int tw /*= None*/, int item_id /*= -1*/)
{
    ...
    if (item_id < 0 && tw != Towards_None)
    {
        auto* gu = GameUtil::getInstance();
        if (submap_id_ == 70 && gu->getInt("shadow_dongfang", "enable", 1) != 0)
        {
            const int shadow_idx = gu->getInt("shadow_dongfang", "clone_role_index", 316);
            if (!Event::getInstance()->inTeam(shadow_idx))
            {
                ...
                if (x == shx && y == shy)
                {
                    return Event::getInstance()->callEvent(1018, this, submap_info_->ID, item_id, -1, x, y);
                }
            }
        }
        // 令狐冲的剑魂 NPC
        if (checkSwordSoulNPC(x, y))
        {
            return true;
        }
    }
...
void SubScene::drawSwordSoulNPC(int ix, int iy, int px, int py, int h, std::vector<Color> color_v, std::vector<float> brighness_v)
{
    if (submap_id_ != 70) { return; }
    auto* gu = GameUtil::getInstance();
    if (gu->getInt("sword_soul_npc", "enable", 1) == 0) { return; }
    // 剑魂已入队则不绘制
    int soul_slot = gu->getInt("sword_soul_npc", "clone_role_index", 317);
    if (Event::getInstance()->inTeam(soul_slot)) { return; }
    int npc_x = gu->getInt("sword_soul_npc", "tile_x", 28);
    int npc_y = gu->getInt("sword_soul_npc", "tile_y", 35);
    if (ix != npc_x || iy != npc_y) { return; }

    // 渲染剑魂 NPC：优先使用 head 头像（辨识度高），备选 smap/fight 贴图
    std::string tex_path = gu->getString("sword_soul_npc", "sprite_path", "");
    int pic = 0;
    if (tex_path.empty())
    {
        // 兼容旧配置：sprite_pic_doubled
        int pic_doubled = gu->getInt("sword_soul_npc", "sprite_pic_doubled", 0);
        pic = pic_doubled / 2;
        tex_path = "smap";
    }
    // 半透明魂体效果，带蓝绿色调
    TextureRenderInfo info;
    info.c = { 100, 200, 255, 180 };
    info.alpha = 160;
    info.zoom_x = 1;
    info.zoom_y = 1;
    info.angle = 0;
    info.white = 0;
    info.color_v = color_v;
    info.brightness_v = brighness_v;

    if (tex_path == "head")
    {
        // 使用角色头像渲染（HeadID），缩放至地图精灵大小
        int head_id = gu->getInt("sword_soul_npc", "head_id", -1);
        if (head_id < 0)
        {
            // 从克隆角色读取 HeadID
            auto role = Save::getInstance()->getRole(soul_slot);
            head_id = role ? role->HeadID : 35;
        }
        info.zoom_x = 0.8;
        info.zoom_y = 0.8;
        TextureManager::getInstance()->renderTexture("head", head_id, px, py - 20, info);
        // 绘制名称
        auto font = Font::getInstance();
        std::string name = gu->getString("sword_soul_npc", "name", "令狐沖的劍魂");
        font->draw(name, 16, px - 10, py + 30, { 100, 200, 255, 220 });
    }
    else if (tex_path != "smap" || pic > 0)
    {
        TextureManager::getInstance()->renderTexture(tex_path, pic, px, py, info);
    }
}

bool SubScene::checkSwordSoulNPC(int x, int y)
{
    if (submap_id_ != 70) { return false; }
    auto* gu = GameUtil::getInstance();
    if (gu->getInt("sword_soul_npc", "enable", 1) == 0) { return false; }
    // 剑魂已入队则不触发
    int soul_slot = gu->getInt("sword_soul_npc", "clone_role_index", 317);
    if (Event::getInstance()->inTeam(soul_slot)) { return false; }
    int npc_x = gu->getInt("sword_soul_npc", "tile_x", 28);
    int npc_y = gu->getInt("sword_soul_npc", "tile_y", 35);
    if (x != npc_x || y != npc_y) { return false; }
    // 调用 Lua 事件脚本 ka1019.lua
    return Event::getInstance()->callEvent(1019, this, submap_info_->ID, -1, -1, x, y);
}

void SubScene::drawMiniMap()
{
    if (!submap_info_) { return; }

    auto engine = Engine::getInstance();
    int screen_w = engine->getUIWidth();
    int screen_h = engine->getUIHeight();

    const int tile_size = 2;  // 每个 tile 2x2 像素
    const int map_size = SUBMAP_COORD_COUNT * tile_size;  // 128x128
    const int margin = 10;
    int map_x = screen_w - map_size - margin;
    int map_y = margin + 70;  // 避开坐标显示区

    // 半透明背景
    engine->fillColor({ 0, 0, 0, 180 }, map_x - 2, map_y - 2, map_size + 4, map_size + 4);

    for (int y = 0; y < SUBMAP_COORD_COUNT; y++)
    {
        for (int x = 0; x < SUBMAP_COORD_COUNT; x++)
        {
            Color c = { 40, 40, 40, 255 };  // 默认深灰（不可达区域）

            bool building = isBuilding(x, y);
            bool water = isWater(x, y);
            bool walk = canWalk(x, y);
            bool exit_tile = (submap_info_->ExitX[0] == x && submap_info_->ExitY[0] == y)
                         || (submap_info_->ExitX[1] == x && submap_info_->ExitY[1] == y)
                         || (submap_info_->ExitX[2] == x && submap_info_->ExitY[2] == y);

            if (exit_tile)
            {
                c = { 255, 255, 0, 255 };  // 出口：黄色
            }
            else if (water)
            {
                c = { 30, 100, 200, 255 };  // 水：蓝色
            }
            else if (building)
            {
            ...
            {
                c = { 34, 100, 34, 255 };  // 可走：深绿
            }

            engine->fillColor(c, map_x + x * tile_size, map_y + y * tile_size, tile_size, tile_size);
        }
    }

    // 主角位置：红色圆点（3x3）
    int px = map_x + man_x_ * tile_size - 1;
    int py = map_y + man_y_ * tile_size - 1;
    engine->fillColor({ 255, 50, 50, 255 }, px, py, 4, 4);
    ...
    for (int y = 0; y < SUBMAP_COORD_COUNT; y++)
    {
        for (int x = 0; x < SUBMAP_COORD_COUNT; x++)
        {
            auto evt = submap_info_->Event(x, y);
            if (evt && evt->Event1 > 0)
            {
                bool is_exit = (submap_info_->ExitX[0] == x && submap_info_->ExitY[0] == y)
                            || (submap_info_->ExitX[1] == x && submap_info_->ExitY[1] == y)
                            || (submap_info_->ExitX[2] == x && submap_info_->ExitY[2] == y);
                if (!is_exit)
                {
                    engine->fillColor({ 0, 200, 255, 255 }, map_x + x * tile_size, map_y + y * tile_size, tile_size, tile_size);
                }
            }
        }
    }
    ...
    // 标题
    Font::getInstance()->draw("小地图 (M)", 16, map_x, map_y - 20, { 255, 255, 255, 255 });
}
