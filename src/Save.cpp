...
void Save::cloneRoleFromTo(int dst_index, int src_index, const std::string& new_name, const std::string& new_nick)
{
    if (dst_index < 0 || dst_index >= int(roles_mem_.size()))
    {
        return;
    }
    if (src_index < 0 || src_index >= int(roles_mem_.size()))
    {
        return;
    }
    Role* dstp = getRole(dst_index);
    Role* srcp = getRole(src_index);
    if (!dstp || !srcp)
    {
        return;
    }
    for (auto it = roles_by_name_.begin(); it != roles_by_name_.end();)
    {
        if (it->second == dstp)
        {
            it = roles_by_name_.erase(it);
        }
        else
        {
            ++it;
