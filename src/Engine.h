...
public:
    ...
    /** finger 坐标为 SDL 报告的 0～1（相对窗口）；与高清屏下 Present 矩形同为渲染坐标后再映射到 UI */
    bool fingerNormalizedToUISpace(float nx, float ny, int& ux, int& uy, bool clamp = false) const;
...
private:
    uint64_t time_;
    static double game_speed_;

public:
    static void setGameSpeed(double s) { game_speed_ = std::max(0.1, s); }
    static double getGameSpeed() { return game_speed_; }
...
public:
    ...
    // Android：从 APK assets 解压；iOS：从 App bundle 内的 game.zip 解压到偏好目录。
