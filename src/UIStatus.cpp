...
void UIStatus::draw()
{
    ...
    font->draw("命中", font_size, x, y + 50, color_ability1);
    font->draw(std::format("{:5}", role_->HitRate), font_size, x + 44, y + 50, select_color1(role_->HitRate, Role::getMaxValue()->HitRate));
    font->draw("閃避", font_size, x + 200, y + 50, color_ability1);
    font->draw(std::format("{:5}", role_->Dodge), font_size, x + 244, y + 50, select_color1(role_->Dodge, Role::getMaxValue()->Dodge));
    font->draw("暴擊", font_size, x + 400, y + 50, color_ability1);
    font->draw(std::format("{:5}", role_->CriticalRate), font_size, x + 444, y + 50, select_color1(role_->CriticalRate, Role::getMaxValue()->CriticalRate));
